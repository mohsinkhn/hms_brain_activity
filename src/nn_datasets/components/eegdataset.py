...
def load_spec_data(data_dir, spec_id, spec_offset):
    ...
    time = data[:, 0, 0].astype(np.int32)
    start = np.searchsorted(time, spec_offset, side="right")
    data = np.clip(data[start : start + 300, 1:, :], np.exp(-4), np.exp(6))
    data = np.log(data)
    mean = data.mean()
    std = data.std()
    data = (data - mean) / (2 * std + 1e-6)
    return data
